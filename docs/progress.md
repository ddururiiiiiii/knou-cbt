...
| `claude/custom-domain-update` (main에 병합·배포 완료) | 4주차 로드맵 착수: 가비아에서 `knou-cbt.com` 구매 + Render 커스텀 도메인 연결(A 레코드 `@`/`www` → `216.24.57.1`, root→www 301 리다이렉트, SSL은 Google Trust Services로 자동 발급, 연결 직후 www 엣지 라우팅 전파에 몇 분 소요됐으나 정상화 확인). `robots.txt`/`sitemap.xml`의 `knou-cbt.onrender.com` 참조를 `www.knou-cbt.com`으로 교체해 커밋·main 병합·푸시까지 완료. 진행 중 사용자가 "knou-cbt.com 도메인이 방통대(KNOU) 상표/저작권과 법적 문제 없는지" 질문 → 조사 결과 (1) 도메인 자체는 `부정경쟁방지법` 사이버스쿼팅 조항 소지가 있으나 이미 `knou-cbt` 저장소명/패키지명으로 공개돼 있어 증분 리스크는 낮음 (2) **기출문제 원문 저작권이 더 핵심 리스크** — 대법원 판례상 시험문제는 창작성 인정되는 저작물이고 "영리 목적 재배포는 면책 안 됨"이 확립돼 있음, 다만 동일 컨셉(방통대 기출 + 애드센스)의 개인 사이트(`allaclass.tistory.com`)가 오랫동안 문제없이 운영 중인 게 확인돼 실질적 단속 리스크는 낮다고 판단, 사용자가 진행 결정. GA 대시보드에 새 도메인 스트림 추가는 코드 변경 아니라 사용자가 직접 해야 함(미완료). 이어서 Sentry 알림 이메일 실제 발송 테스트 진행: `application-prod.yml`에 통째로 빠져있던 `sentry:` 설정 블록(dsn/environment/traces-sample-rate) 추가 + 검증용 `/admin/debug/test-error`(ROLE_ADMIN 전용, 강제 예외 발생) 엔드포인트 신규 추가, main 병합·배포(커밋 `912e5a8`) 완료. **테스트 결과: 이슈가 Sentry Issues에 전혀 안 잡히고 알림 이메일도 안 옴 — 원인 미해결 상태로 세션 종료.** 다음 세션에서 확인할 것: (1) `/admin/debug/test-error` 접속 시 실제로 500 에러 화면이 떴는지(앱이 예외를 던졌는지 자체 확인) (2) Render 서비스 Environment 탭에 `SENTRY_DSN` 값이 실제로 채워져 있는지 (3) Render Events에서 최신 배포(`912e5a8`)가 Live로 반영됐는지 — DSN이 비어있으면 Sentry SDK가 사실상 비활성 상태로 동작해 아무 것도 전송 안 함. 부수적으로 관리자 비밀번호를 사용자가 순간 분실했다가 직접 찾아서, 준비했던 로컬 bcrypt 해시 생성용 임시 테스트 파일(`PasswordHashGeneratorTest`)은 커밋 전에 삭제함(운영 DB는 건드리지 않음) | 진행 중 (Sentry 알림 미해결) | - | 다음 세션 시작 지점: 위 Sentry 원인 3가지 확인부터. 해결되면 로드맵 4주차 남은 항목(애드센스 신청, GA 새 도메인 스트림 등록·수집 확인)으로 이어가면 됨 | 2026-08-16 |
